...
- While any actuator or buoyancy output is active, the `ESP32` refreshes motion commands periodically; `Minima` forces all outputs off if no valid command frame arrives for `1500 ms`.
...
- Three consecutive MS5837 read failures, or about `250 ms` without a fresh depth sample, invalidate depth hold and force buoyancy output to stop until valid data returns.
- Ultrasonic readings are marked invalid immediately on bad frames or out-of-range values, so auto-navigation only reacts to fresh valid sonar data.
