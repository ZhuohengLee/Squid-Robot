...
// OSR 4096：转换 9.04ms，分辨率 0.2 mbar（≈2mm 深度），对 SLAM 够用
// 原 OSR 8192 转换 18ms，出错窗口翻倍，稳定性差
constexpr uint8_t MS5837_CMD_CONVERT_D1_4096 = 0x48;
constexpr uint8_t MS5837_CMD_CONVERT_D2_4096 = 0x58;

constexpr uint32_t MS5837_RESET_DELAY_MS = 20;
constexpr uint32_t MS5837_CONVERSION_DELAY_MS = 15;  // 9ms + 6ms 余量
constexpr uint32_t SAMPLE_INTERVAL_MS = 200;  // 5 Hz，匹配 SD 记录频率
...
void DepthSensorManager::update() {
    // 简单阻塞读取，200ms 间隔（5Hz）。
    // sensor scheduler 保证调用时 SPI 已空闲，30ms 阻塞不会冲突。

    const uint32_t start = millis();

    if (_sensorReady && (_lastSampleMs == 0 || start - _lastSampleMs >= SAMPLE_INTERVAL_MS)) {
        _lastSampleMs = start;
    ...
    // readMeasurement 阻塞了 ~30ms，millis() 已经前进，必须重新取
    // 否则 now - _lastUpdate 可能 uint32 下溢，误触发 stale
    ...
    // stale 超时自动恢复
    if (_sensorReady && _debugStatus == DEBUG_STALE &&
        _lastUpdate > 0 && (now - _lastUpdate > 5000)) {
        g_dbg->println(F("[Depth] I2C 恢复"));
        Wire.end();
        delay(5);
        Wire.begin(DEPTH_I2C_SDA, DEPTH_I2C_SCL, DEPTH_I2C_FREQ);
        Wire.setTimeOut(50);
        resetSensor();
        _lastSampleMs = 0;
        _lastUpdate = millis();
...
bool DepthSensorManager::readAdc(uint8_t conversionCommand, uint32_t& value, ReadFailure triggerFailure, ReadFailure fetchFailure) {
    const uint32_t deadline = millis() + 500;  // 整个 readAdc 最多 500ms

    for (uint8_t attempt = 0; attempt < ADC_READ_RETRIES; ++attempt) {
        if (millis() > deadline) {
            // 硬超时：I2C 可能卡死，强制复位总线
            g_dbg->println(F("[Depth] I2C 硬超时，复位总线"));
            Wire.end();
            delay(5);
            Wire.begin(DEPTH_I2C_SDA, DEPTH_I2C_SCL, DEPTH_I2C_FREQ);
            Wire.setTimeOut(50);
...
bool DepthSensorManager::readMeasurement(float& pressureMbar, float& temperatureC) {
    uint32_t d1 = 0;
    uint32_t d2 = 0;
    if (!readAdc(MS5837_CMD_CONVERT_D1_4096, d1, READ_D1_TRIGGER_FAILED, READ_D1_FETCH_FAILED) ||
        !readAdc(MS5837_CMD_CONVERT_D2_4096, d2, READ_D2_TRIGGER_FAILED, READ_D2_FETCH_FAILED)) {
        return false;
    }
...
bool DepthSensorManager::readAdcResult(uint32_t& value) {
    // 仅发 ADC_READ 命令并读取 3 字节，不发转换命令、不 delay。
    // 用于非阻塞状态机，转换命令和等待在外部状态完成。
    Wire.beginTransmission(DEPTH_I2C_ADDRESS);
    Wire.write(MS5837_CMD_ADC_READ);
    _lastI2cError = Wire.endTransmission(true);
    if (_lastI2cError != 0) {
