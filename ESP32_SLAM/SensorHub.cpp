...
void SensorHub::displayAll() {
    ...
    if (_imuMgr) {
        ...
        if (_imuMgr->isValid()) {
            ...
            g_dbg->println(F(" deg"));
            if (_imuMgr->hasGyro()) {
                g_dbg->print(F("  gyro: x="));  g_dbg->print(_imuMgr->getGyroX(), 2);
                g_dbg->print(F(" y="));          g_dbg->print(_imuMgr->getGyroY(), 2);
                g_dbg->print(F(" z="));          g_dbg->print(_imuMgr->getGyroZ(), 2);
                g_dbg->println(F(" deg/s"));
            } else {
                g_dbg->print(F("  gyro: 未启用 (sog1 无效)  原始帧='"));
                g_dbg->print(_imuMgr->getLastRawLine());
                g_dbg->println(F("'"));
            }
        } else {
            g_dbg->print(F("⚠ 传感器故障 ("));
