...
bool SDLogger::startSession(const char* folderName) {
    ...
    _sensorFile.println(F("millis,depth_cm,vz_cms,az_cms2,us_front_cm,us_left_cm,us_right_cm,motion,batt_v,roll_deg,pitch_deg,yaw_deg,target_cm,ctrl_u,buoy_dir,buoy_pwm,hold"));
    _sensorFile.flush();
    _rowsSinceFlush = 0;
...
void SDLogger::logSensor(uint32_t ms,
                          ...
                          float targetCm, float ctrlU,
                          uint8_t buoyDir, uint8_t buoyPwm, bool hold) {
    if (!_sessionActive || !_sensorFile) return;

    _sensorFile.print(ms); _sensorFile.print(',');

    // wfPositive: 给本质非负的量（距离/电压）用，<0 视为无效
    // wfSigned:   给可正可负的量（深度/速度/加速度/控制量/IMU 角度）用，NaN 视为无效
    auto wfPositive = [&](float v, uint8_t d) {
        if (v < 0.0f) _sensorFile.print(F("--")); else _sensorFile.print(v, d);
        _sensorFile.print(',');
    };
    auto wfSigned = [&](float v, uint8_t d) {
        if (isnan(v)) _sensorFile.print(F("--")); else _sensorFile.print(v, d);
        _sensorFile.print(',');
    };

    // 深度/速度/加速度本质是有符号的（出水时深度可负、上浮时速度可负），
    // 必须用 wfSigned，否则调 PID 时所有上浮过程的数据会被错误地标记成无效。
    wfSigned(depthCm, 2); wfSigned(vzCms, 2); wfSigned(azCms2, 2);
    wfPositive(usFront, 1); wfPositive(usLeft, 1); wfPositive(usRight, 1);
    _sensorFile.print(motionStatus); _sensorFile.print(',');
    if (battV < 0.0f) _sensorFile.print(F("--")); else _sensorFile.print(battV, 2);
    _sensorFile.print(',');
    wfSigned(roll, 2); wfSigned(pitch, 2); wfSigned(yaw, 2);

    // PID 调参字段
    wfSigned(targetCm, 2);                 // setpoint
    wfSigned(ctrlU, 3);                    // 控制器原始输出
    _sensorFile.print(buoyDir); _sensorFile.print(',');
    _sensorFile.print(buoyPwm); _sensorFile.print(',');
    _sensorFile.print(hold ? 1 : 0);
    _sensorFile.println();
