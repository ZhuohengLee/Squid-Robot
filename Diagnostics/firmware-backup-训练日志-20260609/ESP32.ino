...
void loop() {
    ...
    if (sdLogger.hasSession() && nowMs - gLastLogMs >= SD_LOG_INTERVAL_MS) {
        ...
        sdLogger.logSensor(
            nowMs,
            // 深度/速度/加速度：可正可负 → 无效用 NaN
            depthValid ? depthMgr.getDepthCm()        : NAN,
            depthValid ? depthMgr.getDepthSpeedCmS()  : NAN,
            depthValid ? depthMgr.getDepthAccelCmS2() : NAN,
            // 超声波/电池：物理非负 → 无效仍用 -1.0f
            ...
            imuOk ? imuMgr.getRoll()  : NAN,
            imuOk ? imuMgr.getPitch() : NAN,
            imuOk ? imuMgr.getYaw()   : NAN,
            // PID 调参字段：setpoint 在未 hold 时不写绝对值，传 NaN
            depthHolding ? depthController.getTargetDepthCm() : NAN,
            depthController.getControlOutput(),
